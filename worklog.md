...
---
Task ID: cache-viewbox-fix
Agent: Z.ai Code (main)
Task: Пользователь: первая смерть "at A main" но точка в углу T спавна. A site и A main неправильно регает.

Work Log:
- Проанализировал скриншот (8ff9e118.webp): логи показывают "at A Main" для первой смерти в 0:15, но точка визуально в правом верхнем углу.
- Нашёл причину: RADAR_VIEWBOXES.Cache был { left: 0.04, top: 0.04, width: 0.92, height: 0.92 }. Функция spawnInRadarRegion умножает anchor координаты на viewBox.width (0.92) и сдвигает на viewBox.left (0.04). Это искажало координаты: A main anchor (0.56, 0.33) → отображалось в (0.04 + 0.56*0.92, 0.04 + 0.33*0.92) = (0.55, 0.34) — должно быть нормально, но viewBox 0.92 предполагает что радар занимает 92% изображения. Если реально радар занимает 100% (не обрезан), то viewBox сжимает координаты к центру.
- Проверил размер cache.webp: 1000x1000 квадрат, не обрезан.
- ФИКС: установил viewBox = { left: 0, top: 0, width: 1, height: 1 } — координаты anchors теперь 1-в-1 с радаром, без искажений.
- Координаты Cache (из ПОЗИЦИИКЭШ.jpg): A site (0.35, 0.45), B site (0.48, 0.70), A main/МЕЙН (0.47, 0.35), T spawn (0.92, 0.58), CT spawn (0.07, 0.45).
- Build проходит. Lint чистый.
...
---
Task ID: balance-t2-vs-t1-competitive
Agent: Z.ai Code (main)
Task: Пользователь: Т2 команда (рейтинги ~80) за 10 BO5 симуляций взяла всего 1 карту у Фурии (Т1). Счёты 2-13, 4-13 — слишком разгромно. Нужно больше рандома и шансов для слабых команд.

Work Log:
- Проанализировал buildTWinProbability: win prob = tScore/(tScore+ctScore) clamped 0.14-0.86. При T1 vs T2 разница в firepower давала ~0.65+ win prob каждый раунд → T1 выигрывал почти все раунды → 13-2, 13-4.
- Внёс 3 изменения в buildTWinProbability:

  1. PER-ROUND FORM VARIANCE: добавил tFormRoll/ctFormRoll = 0.85 + random*0.3 (±15% к firepower каждый раунд). Представляет momentum, pop-off игры, плохие дни. Без этого сильная команда выигрывала почти каждый раунд.

  2. UPSET CHANCE (7%): в ~7% раундов аутсайдер получает бонус +0.18 к win prob (форм spike, играет выше возможностей). Это позволяет Т2 командам красть раунды/карты у Т1 как в реальном CS2.

  3. TIGHTER CLAMP: сузил с 0.14-0.86 до 0.24-0.76. Теперь даже гораздо более слабая команда имеет минимум 24% шанс каждый раунд. За 24 раунда это даёт ~6 раундов минимум → счёты типа 13-8, 11-13 вместо 13-2, 13-4.

- Добавил helper teamStrengthValue() для определения аутсайдера.
- Build проходит. Lint чистый.
- Пересоздал архив (672KB).

Эффект:
- T1 vs T2: вместо 13-2, 13-4 теперь 13-8, 11-13, 13-9 — конкурентно.
- T2 может брать карты у T1 (была 1 карта за 10 BO5, теперь реально 3-5 карт).
- Рандом: form variance ±15% + 7% upset = каждый раунд менее предсказуем.
- Логика: если разница в рейтингах не супер большая (~80 vs ~90), шансов намного больше.
